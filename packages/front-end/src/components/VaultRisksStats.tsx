...
import { DECIMALS, DHV_NAME } from "../config/constants";
...
export const VaultRisksStats = () => {
  ...
  // const [delta, setDelta] = useState<BigNumber | null>(null);
  ...
  //   if (lpContract) {
  //     const portfolioDelta = await lpContract.getPortfolioDelta();
  //     console.log( portfolioDelta )
  //     setDelta(portfolioDelta);
  //   }
  // }, [lpContract]);
        ...
        <div className="flex flex-col items-left justify-center h-full mb-8 lg:mb-0">
          <p className="mb-2 text-xl">
            ...
            {/* { delta &&
              <>
                <NumberFormat
                  value={ ethers.utils.formatUnits(delta, DECIMALS.RYSK) }
                  displayType={"text"}
                  decimalScale={2}
                  suffix="%"
