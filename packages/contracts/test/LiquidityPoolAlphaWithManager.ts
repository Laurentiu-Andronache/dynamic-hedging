...
describe("Liquidity Pool with alpha tests", async () => {
	before(async function () {
		...
		authority = deployParams.authority
	...
	describe("Setup manager contract as manager", async () => {
		it("SETUP: set manager as manager contract", async () => {
			...
			expect(await authority.manager()).to.equal(manager.address)
			await manager.setProxyManager(senderAddress)
			expect(await manager.proxyManager()).to.equal(senderAddress)
		})
	})
	describe("Deploys rage trade", async () => {
		it("SETUP: deploys rage", async () => {
		...
		it("SETUP: deploys the hedging reactor", async () => {
		...
		it("SETUP: deploy range order", async () => {
	...
	describe("Hedging logic", async () => {
		it("SUCCEEDS: hedges positive delta in perp hedging reactor normally", async () => {
			const delta = toWei("2")
			...
			expect(newDelta).to.equal(toWei("-2"))
		...
		it("REVERTS: hedges positive delta in perp hedging reactor via proxy manager with no delta limit", async () => {
			...
			).to.be.revertedWithCustomError(manager, "ExceedsDeltaLimit")
		})
		it("REVERTS: hedges delta in perp hedging reactor via non keeper", async () => {
			...
			).to.be.revertedWithCustomError(manager, "NotKeeper")
		})
	...
	describe("Create and execute a single buy order", async () => {
		...
		it("REVERTS: Cant make a buy order via manager if not admin", async () => {
			const [sender, receiver] = signers
			const priceQuote = await priceFeed.getNormalizedRate(weth.address, usd.address)
			const collateralAllocatedBefore = await liquidityPool.collateralAllocated()
			const lpUSDBalanceBefore = await usd.balanceOf(liquidityPool.address)
		...
		it("REVERTS: Cant create buy order if price is zero", async () => {
			...
			await expect(
				manager.createOrder(proposedSeries, amount, pricePer, orderExpiry, receiverAddress, false, [
					toWei("1"),
					toWei("1")
				])
	...
	describe("Create and execute a strangle", async () => {
		...
		let strangleCallId: any
		let stranglePutId: any
		let strangleCallToken: IOToken
		let stranglePutToken: IOToken
		it("SUCCEEDS: creates a custom strangle order via manager", async () => {
			let customOrderPriceMultiplier = 1
			const [sender, receiver] = signers
			const priceQuote = await priceFeed.getNormalizedRate(weth.address, usd.address)
			...
			const orderIds = await handler.callStatic.createStrangle(
				proposedSeriesCall,
				proposedSeriesPut,
				amount,
				amount,
			...
			const createStrangle = await manager.createStrangle(
				proposedSeriesCall,
				proposedSeriesPut,
				amount,
				amount,
			...
			strangleCallId = orderIds[1]
			stranglePutId = orderIds[0]
	...
	describe("Create and execute a single buyback order via manager", async () => {
		let optionToken: IOToken
		let customOrderPrice: number
		...
		it("SUCCEEDS: Creates a buyback order via manager", async () => {
			let customOrderPriceMultiplier = 1
			const [sender, receiver] = signers
			const priceQuote = await priceFeed.getNormalizedRate(weth.address, usd.address)
			const collateralAllocatedBefore = await liquidityPool.collateralAllocated()
		...
		it("REVERTS: Cant make a buyback order if not admin of manager", async () => {
			const [sender, receiver] = signers
			const priceQuote = await priceFeed.getNormalizedRate(weth.address, usd.address)
			const collateralAllocatedBefore = await liquidityPool.collateralAllocated()
			const lpUSDBalanceBefore = await usd.balanceOf(liquidityPool.address)
		...
		it("REVERTS: Cant create buyback order if price is zero", async () => {
			...
			await expect(
				manager.createOrder(proposedSeries, amount, pricePer, orderExpiry, receiverAddress, true, [
					toWei("1"),
					toWei("1")
				])
			).to.be.revertedWithCustomError(handler, "InvalidPrice")
